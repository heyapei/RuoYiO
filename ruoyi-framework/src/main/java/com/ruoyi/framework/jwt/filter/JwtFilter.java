    ...
    protected boolean executeLogin(ServletRequest request, ServletResponse response) throws Exception {
        ...
        try {
            ...
            if (e.getCause() instanceof TokenExpiredException) {
                StringRedisTemplate stringRedisTemplate = getBean(StringRedisTemplate.class, (HttpServletRequest) request);
                String tokenKey = REDIS_TOKEN_KEY_PRE + token;
                String cacheToken = String.valueOf(stringRedisTemplate.opsForValue().get(tokenKey));
                if (cacheToken != null && cacheToken.length() != 0 && !cacheToken.equalsIgnoreCase("null")) {
//                    String message = String.format("续期时间：%s；",
//                            DateUtils.dateTimeNow(DateUtils.YYYY_MM_DD_HH_MM_SS)) + cacheToken;
                    String message = cacheToken;
                    stringRedisTemplate.opsForValue().set(tokenKey, message);
                    long expireTime = EXPIRE_TIME / 1000;
